...
3. 면접에서 활용할 수 있는 구체적 키워드·사례"""
    ...
    def generate_swot_list(
        ...
        career_context = (
            "지원자는 신입 지원자입니다. 실무 경험이 제한적임을 전제로, 성장 가능성·학습 의지·잠재력·학업 성취를 강점으로 보고, 실무 경험 부재를 약점으로 평가하세요."
            if career_level == "신입" else
            "지원자는 경력직 지원자입니다. 즉시 전력 여부·전문성·성과·실무 역량을 중심으로 평가하고, 이직 사유와 경력 활용도를 고려하세요."
        )
...
- **[키워드 3]**: [이 키워드를 면접에서 어떻게 활용하면 좋은지 1문장]"""
    ...
    def generate_final_report(
        ...
        career_instruction = (
            "지원자는 신입입니다. 면접 질문과 권고사항은 실무 경험 대신 학습 의지·성장 가능성·잠재력·학업 성취를 부각하는 방향으로 작성하세요. 경력직 수준의 역량을 요구하지 마세요."
            if career_level == "신입" else
...
[지원자만의 차별점과 면접에서 집중 강조할 핵심 포인트 2~3문장]"""
    ...
    def analyze_resume(self, resume: str) -> Dict[str, Any]:
        ...
        지원자의 핵심 정보를 구조화합니다.
        반드시 유효한 JSON만 응답하세요. 설명 텍스트, 마크다운 펜스, 주석, trailing comma 없이
        순수 JSON 객체만 출력하세요."""
