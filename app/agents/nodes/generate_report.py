...
async def generate_report(state: AnalysisState) -> AnalysisState:
    ...
    try:
        swot_message = await claude_llm_service.client.messages.create(
        ...
        raw_swot = swot_message.content[0].text.strip()
    ...
    try:
        ...
        report_message = await claude_llm_service.client.messages.create(
            model="claude-sonnet-4-5-20250929",
            max_tokens=2000,
            messages=[
                {
        ...
        final_report = report_message.content[0].text
